...
class OurModelCustom(nn.Module):
    def __init__(self, Nc, Ns, Nt, c2v, n2v, Ms, cmask, smask, Ac, As, 
                 d_mv=7, d_ms=9, d_v=64, d_p=8, d_h=64, d_f=256, heads=2):
        super(OurModelCustom, self).__init__()
        

        import json
        with open('subdict.json') as fp:
            self.subdict = json.load(fp)
        ...
        self.model_embM = nn.Sequential(nn.Linear(self.d_mm, self.d_h), nn.ReLU(), nn.Linear(self.d_h, self.d_h//2), nn.Softmax(dim=-1))
    ...
    def forward(self, m, c, p):
        ...
        mv = []
        for i in range(7):
            mv.append(m[:, :, self.subdict[str(i)]].sum(axis=-1))
        mv = torch.stack(mv)
        mv = mv.permute([1, 2, 0])
        m = torch.cat((mv, ms), dim=-1)

        embM = self.model_embM(m).view(bs, self.Ns, 1, self.d_h//2).repeat(1, 1, self.Nt, 1)
        ...
        embMT = torch.cat((embM, embT4S), dim=-1)  
