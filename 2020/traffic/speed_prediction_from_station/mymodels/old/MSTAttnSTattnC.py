...
class MSTAttnSTattnC(nn.Module):
    def __init__(self, Nc, Ns, Nt, c2v, n2v, Ms, cmask, smask, 
        ...
        self.model_embMST = nn.Sequential(
            nn.Linear(self.d_h*3, self.d_h),
            nn.ReLU(),
        )
    ...
    def forward(self, m, c, p):
        ...
        embMST = self.model_embMST(torch.cat((embMS, embST), dim=-1))
        ...
        es = embMST.permute(1, 0, 2, 3).reshape(self.Ns, bs*self.Nt, self.d_h)
        ...
        x = s
        x = self.model_ST1(torch.cat((self.attnS1(x, embST), self.attnT1(x, embST)), dim=-1))
