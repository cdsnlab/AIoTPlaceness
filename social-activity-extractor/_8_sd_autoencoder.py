    ...
    try:
        ...
        for fold_idx in range(args.fold):
            ...
            if args.resume:
                ...
                sdae.load_model(os.path.join(CONFIG.CHECKPOINT_PATH, args.prefix + "_" + args.target_modal + "_sdae_" + str(fold_idx)) + ".pt")
            else:
            ...
            sdae.fit(train_loader, val_loader, lr=args.lr, num_epochs=args.epochs, corrupt=0.2, loss_type="mse",
                     save_path=os.path.join(CONFIG.CHECKPOINT_PATH, args.prefix + "_" + args.target_modal + "_sdae_" + str(fold_idx)) + ".pt")
    finally:
        exp.end()
