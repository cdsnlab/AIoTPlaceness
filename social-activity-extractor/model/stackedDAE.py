...
class StackedDAE(nn.Module):
    def __init__(self, device, input_dim=784, z_dim=10, binary=True,
    ...
    def pretrain(self, trainloader, validloader, lr=0.001, batch_size=128, num_epochs=10, corrupt=0.2,
        ...
        for l in range(1, len(self.layers)):
            ...
            if l != len(self.layers) - 1:
                dae = DenoisingAutoencoder(self.device, infeatures, outfeatures, activation=self.activation, dropout=corrupt)
            else:
                dae = DenoisingAutoencoder(self.device, infeatures, outfeatures, activation="none", dropout=0)
            print(dae)
            if l == 1:
