...
def get_output(z, emb, model_name="model"):
    ...
    emb_updated = pd.DataFrame(emb[['Reporting Month', 'ADM_NM']], columns=['Reporting Month', 'ADM_NM'])  # ADM_NM 열만 복사
...
def main(llm_name):
    ...
    # 전체 기본 임베딩 불러오기
    hf_emb = pd.read_csv(f'../../Dataset/AirBnB_LLM_New/{llm_name}_fill_na_ver.csv')
    group = hf_emb.groupby(['Reporting Month'])
    
    # 그래프 기본 경로 설정
    basic_path = f'../../Dataset/AirBnB_Graph/{llm_name}/'
    ...
    for key, df in group:
        ...
        # 학습된 임베딩 저장
        gcn_path = f"../../Dataset/GCN_Embeddings/GCN_AirBnB_{llm_name}_Embeddings.csv"
        gat_path = f"../../Dataset/GCN_Embeddings/GAT_AirBnB_{llm_name}_Embeddings.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Input llm name")
    # 파라미터 추가
    parser.add_argument('--llm_name', type=str, default='llama3_no_listing_raw', help="First parameter")
    
    args = parser.parse_args()
    
    llm_name = args.llm_name   # 파일 명 저장
    main(llm_name)