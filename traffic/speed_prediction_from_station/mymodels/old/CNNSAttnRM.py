...
class CNNSAttnRM(nn.Module):
    def __init__(self, Nc, Ns, Nt, c2v, n2v, Ms, cmask, smask, 
        ...
        self.model_cnn = torch.nn.Conv1d(2, self.d_h, kernel_size=5, padding=2)
    ...
    def forward(self, m, c, p):
        ...
        c = c.permute(0, 1, 3, 2)
        c = c.view(bs*self.Nc, 2, self.Nt)
        c = self.model_cnn(c)   
        c = c.view(bs, Nc, self.d_h, self.Nt)
        c = c.permute(0, 1, 3, 2)
