...
class G3CSAttnRM(nn.Module):
    def __init__(self, Nc, Ns, Nt, c2v, n2v, Ms, Ac, As, cmask, smask, 
                 d_m=10, d_ms=9, d_v=64, d_h=90, d_f=256, d_p=8, heads=1):
        super(G3CSAttnRM, self).__init__()
        ...
        for i in range(self.Ns):
            for j in range(self.Nc):
                ista = max(0, (i-2)*self.Nt)
                iend = min(self.Ns*self.Nt, (i+2)*self.Nt)
                jsta = max(0, (j-2)*self.Nt)
                jend = min(self.Ns*self.Nt, (j+2)*self.Nt)
        ...
        self.model_embMMST = nn.Sequential(
            nn.Linear(self.d_h*3, self.d_h*2),
            nn.ReLU(),
            nn.Linear(self.d_h*2, self.d_h),
            nn.ReLU(),
        )
