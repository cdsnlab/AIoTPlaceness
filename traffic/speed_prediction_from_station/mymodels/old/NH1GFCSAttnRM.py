...
class NH1GFCSAttnRM(nn.Module):
    def __init__(self, Nc, Ns, Nt, c2v, n2v, Ms, Ac, As, cmask, smask,
                 d_m=10, d_ms=9, d_v=64, d_h=56, d_f=256, d_p=8, heads=1):
        super(NH1GFCSAttnRM, self).__init__()
        ...
        self.GSW1 = Parameter(torch.randn(self.Ns*self.Nt, self.Ns*self.Nt), requires_grad=True)
        self.GSB1 = Parameter(torch.randn(self.Ns*self.Nt, self.d_h), requires_grad=True)
        self.GSL1 = nn.Linear(self.d_h, self.d_h, bias=False)
        self.GSW2 = Parameter(torch.randn(self.Ns*self.Nt, self.Ns*self.Nt), requires_grad=True)
        self.GSB2 = Parameter(torch.randn(self.Ns*self.Nt, self.d_h), requires_grad=True)
        self.GSL2 = nn.Linear(self.d_h, self.d_h, bias=False)
        self.GSW3 = Parameter(torch.randn(self.Ns*self.Nt, self.Ns*self.Nt), requires_grad=True)
        self.GSB3 = Parameter(torch.randn(self.Ns*self.Nt, self.d_h), requires_grad=True)
        ...
        self.Smask = Parameter(torch.eye(self.Nt).repeat_interleave(self.Ns, dim=0).repeat_interleave(self.Ns, dim=1), requires_grad=False)
    ...
    def forward(self, m, c, p):
        ...
        cmasks = self.Ac.repeat(self.Nt, self.Nt) * self.Gmask
        #scores = self.GCW.masked_fill(cmasks == 0, -1e9)
        #scores = F.softmax(scores, dim=-1)
        c = c.view(bs, self.Nc*self.Nt, self.d_h)
        c = F.relu(cmasks @ self.GCL(c) + self.GCB)
        s = c.view(bs, self.Ns, self.Nt, self.d_h)
        
        
        smasks = self.As.repeat(self.Nt, self.Nt) * self.Smask
        #scores = self.GSW1.masked_fill(smasks == 0, -1e9)
        #scores = F.softmax(scores, dim=-1)
        s = s.view(bs, self.Ns*self.Nt, self.d_h)
        s = F.relu(smasks @ self.GSL1(s) + self.GSB1)
        s = s.view(bs, self.Ns, self.Nt, self.d_h)

        smasks = self.As.repeat(self.Nt, self.Nt) * self.Smask
        #scores = self.GSW2.masked_fill(smasks == 0, -1e9)
        #scores = F.softmax(scores, dim=-1)
        s = s.view(bs, self.Ns*self.Nt, self.d_h)
        s = F.relu(smasks @ self.GSL2(s) + self.GSB2)
        s = s.view(bs, self.Ns, self.Nt, self.d_h)

        smasks = self.As.repeat(self.Nt, self.Nt) * self.Smask
        #scores = self.GSW3.masked_fill(smasks == 0, -1e9)
        #scores = F.softmax(scores, dim=-1)
        s = s.view(bs, self.Ns*self.Nt, self.d_h)
        s = F.relu(smasks @ self.GSL3(s) + self.GSB3)
        s = s.view(bs, self.Ns, self.Nt, self.d_h)
